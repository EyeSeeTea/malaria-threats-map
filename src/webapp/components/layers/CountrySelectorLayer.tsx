...
class CountrySelectorLayer extends Component<Props> {
    ...
    mountLayer = () => {
        ...
        if (this.props.countries) {
            ...
            const statusByCountry: { [key: string]: any } = Object.entries(studiesByCountry).reduce(
                (acc, [key, studies]) => ({
                    ...acc,
                    [key]: {
            ...
            const features = this.props.countries.features
                ...
                .map((feature: CountryFeature) => {
                    ...
                    const countryStatus: { [key: string]: number } = statusByCountry[newFeature.properties.ISO_2_CODE];
                    if (!countryStatus) {
                        newFeature.properties.PBO_DEPLOYMENT_STATUS = PboDeploymentCountriesStatus.NOT_APPLICABLE;
                        return newFeature;
                    }
                    const statuses: Record<string, number> = Object.entries(countryStatus).reduce(
            ...
            const existing: any = this.props.map.getSource(COUNTRY_SELECTOR_SOURCE_ID);
            if (existing) {
                existing.setData({
            ...
            setupEffects(this.props.map, COUNTRY_SELECTOR_SOURCE_ID, COUNTRY_SELECTOR_LAYER_ID);
            this.setupPopover();
            this.showLayer();
        } else {
            this.hideLayer();
    ...
    showLayer = () => {
        if (this.props.map.getLayer(COUNTRY_SELECTOR_LAYER_ID)) {
            this.props.map.setLayoutProperty(COUNTRY_SELECTOR_LAYER_ID, "visibility", "visible");
        }
    };

    hideLayer = () => {
        if (this.props.map.getLayer(COUNTRY_SELECTOR_LAYER_ID)) {
            this.props.map.setLayoutProperty(COUNTRY_SELECTOR_LAYER_ID, "visibility", "none");
        }
    };
    ...
    setupPopover = () => {
        this.props.map.off("click", COUNTRY_SELECTOR_LAYER_ID, this.onClickListener);
