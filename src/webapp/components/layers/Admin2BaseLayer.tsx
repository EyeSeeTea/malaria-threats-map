...
const ADMIN_2_LAYER_ID = "admin2-layer";
const ADMIN_2_SOURCE_ID = "admin2-source";
...
class Admin1BaseLayer extends Component<Props> {
    componentDidMount(): void {
        ...
        if (this.props.map.getSource(ADMIN_2_SOURCE_ID)) {
            this.showLayer();
        } else {
            this.props.map.addSource(ADMIN_2_SOURCE_ID, source);
            this.props.map.addLayer(layer);
        }

        this.props.map.on("zoomend", () => {
            if (this.props.map.getZoom() >= 5.5) {
    ...
    showLayer = () => {
        if (this.props.map.getLayer(ADMIN_2_LAYER_ID)) {
            this.props.map.setLayoutProperty(ADMIN_2_LAYER_ID, "visibility", "visible");
        }
    };

    hideLayer = () => {
        if (this.props.map.getLayer(ADMIN_2_LAYER_ID)) {
            this.props.map.setLayoutProperty(ADMIN_2_LAYER_ID, "visibility", "none");
        }
    };
    ...
    async executeQuery() {
        ...
        const response = await axios.get(
            "https://services.arcgis.com/5T5nSi527N4F7luB/arcgis/rest/services/Detailed_Boundary_ADM2/FeatureServer/0/query",
            { params }
        );

        (this.props.map.getSource(ADMIN_2_SOURCE_ID) as GeoJSONSource).setData(mapResponseData(response.data));
    }
}
